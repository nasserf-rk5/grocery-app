...
LANGUAGES = {
    "العربية": {
        ...
        "price": "السعر", "cost": "التكلفة", "stock": "المخزون المتوفر", "qty": "الكمية المباعة:",
        "sell_btn": "إتمام البيع وسحب المخزون", "success_sell": "تم البيع بنجاح! الإجمالي",
        "error_stock": "عذراً، الكمية غير متوفرة!", "undo_btn": "↩️ تراجع عن آخر عملية بيع",
        "undo_success": "تم التراجع عن آخر بيعة وإعادة المخزون بنجاح!", "no_sales_to_undo": "لا توجد عمليات بيع للتراجع عنها.",
        "add_item": "إضافة صنف جديد", "item_name": "اسم المنتج:", "item_cat": "الفئة الرئيسية:",
...
with tab1:
    ...
    if categories:
        ...
        if items_in_cat:
            ...
            if st.button(t['sell_btn']):
                if stock >= qty:
                    ...
                    sale_record = {
                        ...
                        "category": selected_cat, "item": selected_item, "qty": qty,
                        "total_price": total_price, "total_cost": total_cost, "profit": profit,
            ...
            st.markdown("---")
            if st.button(t['undo_btn']):
                if len(st.session_state.sales_history) > 0:
                    last_sale = st.session_state.sales_history.pop()
                    c = last_sale["category"]
                    i = last_sale["item"]
                    q = last_sale["qty"]
                    if c in st.session_state.inventory and i in st.session_state.inventory[c]:
                        st.session_state.inventory[c][i]["stock"] += q
                    save_data(st.session_state.inventory)
                    save_sales(st.session_state.sales_history)
